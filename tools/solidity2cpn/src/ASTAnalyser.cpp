...
ASTAnalyser::ASTAnalyser(std::stringstream& _ast_sstream, nlohmann::json& _jsonast, const bool& single_file, const std::string& file_name, const std::string& _visitor_arg) {
    ...
    if (single_file) {
        while (substr_by_edge(*ptr_ast_line, "======= ", " =======") != file_name) {
            ++ptr_ast_line;
        }
    }
...
RootNodePtr ASTAnalyser::analyse() {
    ...
    while (ptr_ast_line != ast_lines.end()) {
        std::string keyword = retrieve_string_element(*ptr_ast_line, 0, " ");
        
        debug_info("Contract-level ast line handling: " + *ptr_ast_line);
        /* if (keyword == TokenPragmaDirective) {
            get_next_token(TokenSource);
            line = substr_by_edge(*ptr_ast_line, "Source: \"", "\"");
            remove_escapes(line);
            pragma << line;
        } else 
        ...
        if (keyword == TokenContractDefinition ) {
            std::string contract_name = retrieve_string_element(*ptr_ast_line, 1, " ");
            contract_name = substr_by_edge(contract_name, "\"", "\"");
            ...
            line = substr_by_edge(*ptr_ast_line, TokenUserDefinedTypeName + " \"", "\"");
            current_contract->add_inherit_from(line);
        } else if (keyword == TokenUsingForDirective ) {
            get_next_token(TokenUserDefinedTypeName);
            std::string using_A = substr_by_edge(*ptr_ast_line, TokenUserDefinedTypeName + " \"", "\""), for_B = "*";
            if (get_next_token() == TokenUserDefinedTypeName) {
                for_B = substr_by_edge(*ptr_ast_line, TokenUserDefinedTypeName + " \"", "\"");
            } else {
            ...
            std::string struct_name = retrieve_string_element(*ptr_ast_line, 1, " ");
            struct_name = substr_by_edge(struct_name, "\"", "\"");
            ...
            std::string function_name = retrieve_string_element(*ptr_ast_line, 1, " ");
            function_name = substr_by_edge(function_name, "\"", "\"");
            ...
            std::string modifier_name = retrieve_string_element(*ptr_ast_line, 1, " ");
            modifier_name = substr_by_edge(modifier_name, "\"", "\"");
            get_next_token(TokenParameterList);
            ParameterListNodePtr params = handle_parameter_list();
            ...
            std::string event_name = retrieve_string_element(*ptr_ast_line, 1, " ");
            event_name = substr_by_edge(event_name, "\"", "\"");
    ...
    debug_info("File processing finished");
...
void ASTAnalyser::get_next_token(const std::string& token) {
    ...
    while (ptr_ast_line != ast_lines.end()) {
        ...
        trim(line);
        if (retrieve_string_element(line, 0, " ") == token) {
            return;
        }
        ++ptr_ast_line;
    }
...
std::string ASTAnalyser::get_next_token() {
    ...
    while (ptr_ast_line != ast_lines.end()) {
        std::string line = *ptr_ast_line;
        trim(line);
        std::string keyword = retrieve_string_element(line, 0, " ");
...
VariableDeclarationNodePtr ASTAnalyser::handle_variable_declaration(){
    ...
    int indentation = get_current_indentation();
    std::string variable_name = substr_by_edge(*ptr_ast_line, TokenVariableDeclaration + " \"", "\"");
    ASTNodePtr type;
    std::string type_str;
...
BinaryOperationNodePtr ASTAnalyser::handle_binary_operation() {
    std::string op = retrieve_string_element(*ptr_ast_line, 3, " ");
    get_next_token(TokenType);
    std::string type_str = retrieve_string_element(*ptr_ast_line, 1, " ");
...
UnaryOperationNodePtr ASTAnalyser::handle_unary_operation(){
    std::string operator_loc = retrieve_string_element(*ptr_ast_line, 1, " ");
    bool is_prefix = operator_loc == "(postfix)"? false : true;
    std::string op = retrieve_string_element(*ptr_ast_line, 2, " ");
    std::string token = get_next_token();
...
LiteralNodePtr ASTAnalyser::handle_literal() {
    get_next_token(TokenSource);
    std::string literal_source = substr_by_edge(*ptr_ast_line, "Source: \"", "\"");
...
IdentifierNodePtr ASTAnalyser::handle_identifier() {
    get_next_token(TokenSource);
    std::string name = substr_by_edge(*ptr_ast_line, "Source: \"", "\"");
...
MemberAccessNodePtr ASTAnalyser::handle_member_access() {
    std::string member = retrieve_string_element(*ptr_ast_line, 3, " ");
    std::string token = get_next_token();
...
ASTNodePtr ASTAnalyser::get_statement_equivalent_node(std::string& token) {
    if (token == TokenBlock) {
        ...
        get_next_token(TokenSource);
        std::string source_line = substr_by_edge(*ptr_ast_line, "Source: \"", "\"");
...
EnumDefinitionNodePtr ASTAnalyser::handle_enum_definition() {
    std::string name = substr_by_edge(*ptr_ast_line, TokenEnumDefinition + " \"", "\"");
...
EnumValueNodePtr ASTAnalyser::handle_enum_value() {
    std::string name = substr_by_edge(*ptr_ast_line, TokenEnumValue + " \"", "\"");
    return std::make_shared<EnumValueNode>(name);
}

PlaceHolderStatementPtr ASTAnalyser::handle_placeholder() {
    get_next_token(TokenSource);
    std::string placeholder = substr_by_edge(*ptr_ast_line, "Source: \"", "\"");
...
ASTNodePtr ASTAnalyser::get_type_name(std::string& token) {
    ...
    if (token == TokenElementaryTypeName) {
                debug_info("elementary");
        ++ptr_ast_line;
        type_str = substr_by_edge(*ptr_ast_line, "Source: \"", "\"");
        debug_info("type:" + type_str);
        ...
        type_str = substr_by_edge(*ptr_ast_line, TokenUserDefinedTypeName + " \"", "\"");
